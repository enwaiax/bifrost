...
func rollbackWarpMessageOutcomeColumns(tx *gorm.DB) error {
	...
	if mg.HasTable(&WarpMessage{}) {
		...
		if err := tx.Model(&WarpMessage{}).
			Where("finish_reason <> '' OR total_tokens <> 0 OR cost <> 0").
			Count(&recorded).Error; err != nil {
			return fmt.Errorf("could not check recorded warp outcomes before rollback: %w", err)
		}
		if recorded > 0 {
			return fmt.Errorf("logs_add_warp_message_outcome_columns is non-rollbackable: %d warp message(s) carry a recorded finish reason or usage, and dropping these columns would destroy that record rather than reverse a schema change; clear the history first if the rollback is genuinely intended", recorded)
		}
	}
	for _, column := range warpMessageOutcomeColumns {
		if !mg.HasColumn(&WarpMessage{}, column.column) {
