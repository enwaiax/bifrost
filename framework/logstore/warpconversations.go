...
type WarpConversationStore interface {
	...
	// DeleteWarpConversationsOlderThan drops every thread last touched before
	// the cutoff, across all owners.
...
//
// No row lock: Position is scoped to this append (see WarpMessage), so nothing
// has to be reserved and two concurrent appends cannot collide on an ordinal.
...
// DeleteWarpConversationsOlderThan drops every thread last touched before the
// cutoff, across all owners.
//
