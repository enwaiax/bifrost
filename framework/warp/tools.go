...
const FilterSchema = `{
  ...
  "properties": {
    ...
    "status": {"type": "array", "items": {"type": "string"}, "maxItems": 50, "description": "success or error."},
    ...
    "content_search": {"type": "string", "maxLength": 500, "description": "Substring match against request and response content."},
...
func parseFilters(raw map[string]any, now time.Time) (*logstore.SearchFilters, error) {
	...
	if value, present := raw["content_search"]; present && value != nil {
