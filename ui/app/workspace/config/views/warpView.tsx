...
export default function WarpView() {
	...
	useEffect(() => {
		...
		setForm({
			...
			// config.temperature is absent for a row saved before this field was
			// always sent - undefined and null both read as "not configured" here,
			// since JSON only sends one of them but either could arrive depending
			// on what wrote the row.
	...
	useEffect(() => {
		if (isBackfillActive || !backfillStatus?.start_time || !backfillStatus?.end_time) return;
					...
					<div className="space-y-4">
						...
						<div className="space-y-2 rounded-sm border p-4">
							<div className="space-y-0.5">
								...
								<p className="text-muted-foreground text-sm">
									The model&apos;s sampling temperature, from {WARP_MIN_TEMPERATURE} to {WARP_MAX_TEMPERATURE}. Most providers default
									near {DEFAULT_TEMPERATURE}. Lower is more consistent answer to answer; a tool that reports numbers often wants that.
								</p>
							</div>
							<Input
								id="warp-temperature"
						...
						<div className="space-y-4 rounded-sm border p-4" data-testid="warp-backfill-section">
							...
							{backfillStatus?.id && (
								<div className="bg-muted/40 space-y-2 rounded-sm p-3 text-sm" data-testid="warp-backfill-status">
									...
									{backfillStatus.message && <p className="text-muted-foreground text-xs">{backfillStatus.message}</p>}
								...
								{isBackfillActive ? (
									...
									<>
										...
										>
											...
											{canResumeBackfill
												? `Resume backfill (${backfillStatus.scanned}/${backfillStatus.total})`
												: "Start backfill"}
										</Button>
									</>
								)}
