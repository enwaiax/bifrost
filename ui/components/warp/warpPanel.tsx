...
export default function WarpPanel() {
	...
	}, [isStreaming, queue, question]);

	if (!warp) return null;

	const openStored = async (conversation: WarpConversation) => {
		// Cleared before anything is awaited. A queued follow-up was typed into the
		// thread being left, and the streaming-to-idle transition that stop()
		// causes would drain it into whichever conversation is selected by then.
		setQueue([]);
